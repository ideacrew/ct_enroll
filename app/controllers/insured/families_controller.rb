class Insured::FamiliesController < FamiliesController
  ...
  def notice_upload_email
    if (@person.consumer_role.present? && @person.consumer_role.can_receive_electronic_communication?) ||
      ...
      UserMailer.generic_notice_alert(@person.first_name, "You have a new message from Access Health CT", @person.work_email_or_best).deliver_now
    end
  end
  ...
  def notice_upload_secure_message(notice, subject)
    ...
    @person.inbox.messages << Message.new(subject: subject, body: body, from: 'Access Health CT')
    @person.save!
  end
